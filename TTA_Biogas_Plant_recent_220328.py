...
df_00 = pd.read_excel("ketep_biogas_data_20220314.xlsx")  # Open Excel file
...
# fimp_01 = pd.DataFrame(data=values, index=keys, columns=["score"]).sort_values(by = "score", ascending=False)
# fimp_01.nlargest(30, columns="score").plot(kind='barh', figsize = (10,8)) ## plot top 40 features
...
sns.distplot(a_scores_train, kde=True, rug=True, bins=100, label="IF score")
...
plt.figure(figsize=(15, 5))
sns.distplot(a_scores_test, kde=True, rug=True, bins=100, label="IF score")
plt.xlabel("Average Path Lengths", fontsize=14)
plt.ylabel("Number of Data Points", fontsize=14)
plt.show()
